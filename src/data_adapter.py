...
def main():
    ...
    for cpc in [4,10]:
        #try:
        summery_results = pd.DataFrame(index=np.arange(0),columns=[])
        ...
        try:
            old_wc_data = pd.read_csv(f'..\\results\\old\\cpc {cpc}-wc_new_XORED_csv.csv',converters={'err_vec': code.str2bitarray})
        except:
            old_wc_data = pd.read_csv(f'..\\results\\old\\cpc {3}-wc_new_XORED_csv.csv',
                                      converters={'err_vec': code.str2bitarray})
            old_wc_data = adjust_sizes(old_wc_data,chip)
        df['cpc'] = cpc
        ...
        df['write_new_wc_DataRed'] = df['write_data'] + df['write_total_red']
        ...
        #new detector #todo: to correct this!
        df['wc_new_read_DataRed'] = df.apply(lambda x: (x.write_new_wc_DataRed ^ x.err_vec), axis=1)
        #df['wc_new_read_ad_red'] = df.apply(lambda x: address_decoder.encode(x.write_data,x.bin_original_address), axis=1)
        df['wc_new_read_data'] = df.apply(lambda x: x.wc_new_read_DataRed[:chip.k], axis=1 )
        df['wc_new_read_total_red'] = df.apply(lambda x: x.wc_new_read_DataRed[chip.k:], axis=1)#df['wc_read_data_red'] ^ df['wc_new_read_ad_red']
        df['wc_new_data_calculated_red'] = df.apply(lambda x: chip.encode(x.wc_new_read_data),axis=1)
        df['wc_new_ad_calculated_red'] = df.apply(lambda x: address_decoder.encode(x.wc_new_read_data,x.bin_original_address),axis=1)
        df['wc_new_read_total_calculated_red'] = df.apply(lambda x: (x.wc_new_ad_calculated_red ^ x.wc_new_data_calculated_red), axis=1)#df['wc_new_read_ad_red'] ^ df['wc_read_data_calculated_red']
        ...
        #collecting data
        summery_results['shift_detector'] =df['only_shift_shift_detected'].value_counts()
        summery_results['old_wc_detector'] =df['wc_old_detector'].value_counts()
        summery_results['new_wc_detector'] = df['wc_new_AD_detector'].value_counts()
        summery_results['combined_err_detector'] = df['combined_detector'].value_counts()
        summery_results.to_csv(f'..\\results\\cpc {cpc}' + " summery.csv")
...
def adjust_sizes(df:pd.DataFrame,chip:pacific):
    pad_num = chip.n - len(df['err_vec'][1])
    if pad_num < 0 :
        df['err_vec'] = df.apply(lambda x: x.err_vec[:chip.n], axis=1)
    else:
        df['err_vec'] = df.apply(lambda x: x.err_vec+(bitarray('0')*pad_num), axis=1)
    return df[df['err_vec'] != (bitarray('0')*chip.n)]
